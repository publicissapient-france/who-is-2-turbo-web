...
export const getSearchedValue = (filters: Filter[]) =>
  filters
    .filter((f: Filter) => f.name === 'search')
...
const isMemberMatchingCriterias = (member: User, filters: Filter[]) => {
  ...
  return filters.map((filter: Filter) => isMemberMatchingCriteria(member, filter)).reduce((a, b) => a && b, true);
};
